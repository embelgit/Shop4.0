...
public class EmployeeDetailsDao {
	...
	public List getAllEmployeeDetailsForEdit(Long empId) {
		...
		Query query = null;
		 ...
		 try {
			 hbu = HibernateUtility.getInstance();
			 session = hbu.getHibernateSession();
			 query = session.createSQLQuery("SELECT first_name, middle_name, last_name, joining_date, email_id, salary, contact_no, address, pin_code, adhaar_number FROM employee_details WHERE pk_empoyee_id ="+empId);
			 list = query.list(); 
		} catch (RuntimeException e) {
			e.printStackTrace();
		}
	...
	try{	
		...
		Query query=session.createSQLQuery("SELECT first_name, middle_name, last_name, joining_date, email_id, salary, contact_no, address, pin_code, adhaar_number FROM employee_details;");
	...
	for (Object[] object : list) {	
		...
		reports.setAdhaarNumber(object[9].toString());

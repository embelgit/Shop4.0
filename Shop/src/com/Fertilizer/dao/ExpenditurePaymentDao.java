...
public class ExpenditurePaymentDao {
	...
	public List<ExpenditurePaymentDetail> getExpensePaymentDetailsForSingleDate(
		...
		try
		{
				hbu = HibernateUtility.getInstance();
		 session = hbu.getHibernateSession();
		 Query query = session.createSQLQuery("select expense_name, service_provide, total_amount, credit, debit, accountant_name,expenditure_payment.insert_date,payment_mode FROM expenditure_details RIGHT JOIN expenditure_payment ON expenditure_details.pk_expense_details_id = expenditure_payment.fk_expense_detail_id WHERE DATE(expenditure_payment.insert_date)=:isInsertDate ");
			...
			for (Object[] object : list) {
				...
				reports.setPaymentMode(object[7].toString());
	...
	public List<ExpenditurePaymentDetail> getExpensePaymentDetailByTwoDates(
		...
		try
		{
			hbu = HibernateUtility.getInstance();
		 session = hbu.getHibernateSession();
		 Query query2 = session.createSQLQuery("select expense_name,  total_amount, credit, debit, accountant_name,expenditure_payment.insert_date,payment_mode FROM expenditure_details RIGHT JOIN expenditure_payment ON expenditure_details.pk_expense_details_id = expenditure_payment.fk_expense_detail_id WHERE expenditure_payment.insert_date BETWEEN '"+fDate+"' AND '"+tDate+"'");
			...
			for (Object[] object : list) {
				...
				reports.setCredit(Double.parseDouble(object[2].toString()));
				reports.setDebit(Double.parseDouble(object[3].toString()));
				reports.setAccountantName(object[4].toString());
				reports.setInsertDate(object[5].toString());
	...
	public List<ExpenditurePaymentDetail> getExpensePaymentDetailByTwoDatesPerName(
		...
		try
		{
			hbu = HibernateUtility.getInstance();
		 session = hbu.getHibernateSession();
		 Query query2 = session.createSQLQuery("select expense_name, service_provide, total_amount, credit, debit, accountant_name,expenditure_payment.insert_date,payment_mode FROM expenditure_details RIGHT JOIN expenditure_payment ON expenditure_details.pk_expense_details_id = expenditure_payment.fk_expense_detail_id WHERE fk_expense_detail_id='"+expenseName+"' AND expenditure_payment.insert_date BETWEEN '"+fDate+"' AND '"+tDate+"'");
			/*query2.setParameter("stDate", fDate);
	        query2.setParameter("edDate", tDate);*/
	        List<Object[]> list = query2.list();
	...
	//get latest Employee Payment ID number in Employee Payment Table
			public List getExpenditureLastTransactionNo() {
				...
				HibernateUtility hbu=null;
				Session session=null;
				List<ExpenditurePaymentBean> Txidlist=null;
				try
				{
					hbu = HibernateUtility.getInstance();
					session = hbu.getHibernateSession();
					Query query = session.createSQLQuery("SELECT trans_No_Expenditure, accountant_name FROM expenditure_payment ORDER BY trans_No_Expenditure DESC LIMIT 1");

					List<Object[]> list = query.list();
					Txidlist= new ArrayList<ExpenditurePaymentBean>(0);
					for (Object[] object : list) {
						
						ExpenditurePaymentBean bean=new ExpenditurePaymentBean();
						bean.setTransNoForExpenditure(Long.parseLong(object[0].toString()));
						bean.setAccountantName(object[1].toString());
						
						Txidlist.add(bean);	 
					}}
				catch(RuntimeException e)
				{
					Log.error("Error in getExpenditureLastTransactionNo()", e);	
				}finally
				{if(session!=null){
					hbu.closeSession(session);	
				}
			...
			public List<ExpenditurePaymentDetail> getExpensePaymentDetailByExpenseType(
					String expenseType) {
				...
				try
				{
					hbu = HibernateUtility.getInstance();
				 session = hbu.getHibernateSession();
				 Query query2 = session.createSQLQuery("select expense_name, service_provide, total_amount, credit, debit, accountant_name,expenditure_payment.insert_date,payment_mode FROM expenditure_details RIGHT JOIN expenditure_payment ON expenditure_details.pk_expense_details_id = expenditure_payment.fk_expense_detail_id WHERE expenditure_payment.expense_type ='"+expenseType+"'");
					/*query2.setParameter("stDate", fDate);
			        query2.setParameter("edDate", tDate);*/
			        List<Object[]> list = query2.list();
			        System.out.println("List Size-----------------> "+list.size());
			...
			public List<ExpenditurePaymentDetail> getExpensePaymentmode(
					String startDate,String endDate,String paymode) {
				...
				try
				{
						hbu = HibernateUtility.getInstance();
				 session = hbu.getHibernateSession();
				 Query query = session.createSQLQuery("select expense_name, service_provide, total_amount, credit, debit, accountant_name,expenditure_payment.insert_date,payment_mode FROM expenditure_details RIGHT JOIN expenditure_payment ON expenditure_details.pk_expense_details_id = expenditure_payment.fk_expense_detail_id WHERE expenditure_payment.insert_date between '"+startDate+"' AND '"+endDate+"' AND payment_mode = '"+paymode+"'");

...
<%
	...
	Long TransactionId = (Long) session.getAttribute("transactionIdSupp");

	String shopid = (String) session.getAttribute("shopid");  
//	String shopName = (String) session.getAttribute("onlyShopName");  
	
	System.out.println("```````````````````  in pdf  shopid - - "+shopid);
	...
	try {
		...
		ResultSet rs = stmt.executeQuery("SELECT sp.trans_No_Supplier,sp.supplierName,sd.city,sp.insert_date,sp.payment_mode,sp.payment from supplier_payment sp LEFT JOIN supplier_details sd ON sp.fk_supplier_id=sd.pk_supplier_id WHERE trans_No_Supplier='"+TransactionId+"'");
		...
		table_cell = new PdfPCell(new Phrase("Supplier Payment Bill Memorandum", Normalfont11));
		...
		table_cell = new PdfPCell(new Phrase("Supplier Payment Invoice", Normalfont11));
		table_cell.setBorder(table_cell.NO_BORDER);
		table_cell.setHorizontalAlignment(Element.ALIGN_CENTER);
		table.addCell(table_cell);

	    table_cell = new PdfPCell(new Phrase("Lic No(S)     : "+Lic_No_Shop, font9NoBold));
		table_cell.setBorder(table_cell.NO_BORDER);
		...
		String transactionNo=rs.getString("trans_No_Supplier");
		...
		String supplieName=rs.getString("supplierName");
		...
		table_cell1 = new PdfPCell(new Phrase("Today we will get your shop account from the rest of the Amount in word, ' "+amtInWord+" ', rupees Only Through "+payment_mode+"  Received it into your account", Normalfont12));
		...
		table_cell1 = new PdfPCell(new Phrase(Footer_Shop_Name, Normalfont12));
		table_cell1.setPaddingLeft(400f);
		table_cell1.setBorder(Rectangle.RIGHT | Rectangle.LEFT);
		table1.addCell(table_cell1);

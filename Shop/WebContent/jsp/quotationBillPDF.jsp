...
response.setContentType("application/pdf");
Long quotationNoForPdf = (Long) session.getAttribute("quotationNoForPdf");
String onlyShopName = (String) session.getAttribute("onlyShopName");

System.out.println("ln no ==>66 quotationNoForPdf ===> "+quotationNoForPdf+" onlyShopName ===> "+onlyShopName);
...
try {
	...
	ResultSet rs = stmt.executeQuery("select fk_customer_id, fb.quotation_no, customer_name, credit_customer_name, fb.product_name, fb.village, fb.company, fb.weight, fb.Without_Tax_Rate, fb.total_per_product, fb.total_all_product, fb.hamali_expense, fb.gross_total, fb.tax_percentage, fb.hsn,fb.igstPercentage, fb.fk_shop_id, s.shop_name,fb.Discount_amount ,fb.sale_price, fb.kg, fb.grams, fb.unit, fb.ltr, fb.mili, fb.quantity, fb.total_per_product, fb.kg, fb.grams, fb.ltr, fb.mili, fb.contact_number, fb.GST_No, s.address, fb.saleDate, fb.freeQuantity, fb.net_pay_amount, fb.discount_per_product_perc,fb.email from quotation fb left join shop_details s on fk_shop_id=pk_shop_id where fb.quotation_no = "+quotationNoForPdf);
	...
	rs.first();
		...
		Long shopPrBillno = rs.getLong("quotation_no");
		...
		System.out.println("rslt ` ``````````````````````````````  "+net_pay_amount+mobileNo+fk_customer_id+credit_customer_name+customer_name+gstNo+shopPrBillno+shopAddress);
		/* if(ShopId == 1)
		{ */
			
			if (fk_customer_id == 0) {
			table_cell = new PdfPCell(new Phrase("QUOTATION BILL", font11));
			table_cell.setBorder(table_cell.NO_BORDER);
			table_cell.setHorizontalAlignment(Element.ALIGN_LEFT);
			table.addCell(table_cell);
			} else {
			table_cell = new PdfPCell(new Phrase("Bill CREDIT Memorandum", font9NoBold));
			table_cell.setBorder(table_cell.NO_BORDER);
			table_cell.setHorizontalAlignment(Element.ALIGN_LEFT);
			table.addCell(table_cell);
		    }
			...
			table_cell = new PdfPCell(new Phrase("Quotation No : "+shopPrBillno, font9NoBold));
...
*/
	...
	System.out.println("line 614`` ` ` ` ` ` ``````  "+quantity+unit+weight+boxqty+ltr+calMili+mili+kg+calGrams+grams+kgAndGrams+ltrAndMili);				
...
//						double taxAmount = (GST / 100) * (total1);
		...
		if (b > 0 && b <= 10)
		{
			...
			table_cell4 = new PdfPCell(new Phrase("    "/*CGST Amount :- " + df.format(halfGstTotal) , Normalfont11*/));
			table_cell4.setHorizontalAlignment(Element.ALIGN_CENTER);
			table_cell4.setBorder(Rectangle.NO_BORDER);
